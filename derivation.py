...
def etaTerms():
    ...
    g0 = (y/h) - Integer(1)/Integer(2)*(y/h)**2
    ...
    pprint(diff(u, x).expand())
    print()
    pprint(diff(u, x).subs(y, h))
    print()
    pprint(diff(diff(u, x).subs(y, h), x))
    print()
    pprint(integrate(g0, (y, 0, h)))
    print()
    pprint((diff(diff(u, x).subs(y, h), x)*integrate(g0, (y, 0, h))).expand())
    print()

    pprint(diff(u, x, 2).expand())
    print()
    pprint(expand(diff(u, x, 2) * g0))
    print()
    pprint(integrate(expand(diff(u, x, 2) * g0), (y, 0, h)).expand())
    print()
    pprint((diff(diff(u, x).subs(y, h), x)*integrate(g0, (y, 0, h))).expand() + 2 * integrate(expand(diff(u, x, 2) * g0), (y, 0, h)).expand())
    print()

    pprint(diff(u, t).expand())
    print()
    pprint((diff(u, t) * g0).expand())
    print()
    pprint(integrate(diff(u, t) * g0, (y, 0, h)).expand())
    print()
    
    pprint(Integer(5)/Integer(2) * ((diff(diff(u, x).subs(y, h), x)*integrate(g0, (y, 0, h))).expand() + 2 * integrate(expand(diff(u, x, 2) * g0), (y, 0, h)).expand()))
    print()

    # These are different
    xMomentumEquation = (e * delta * (diff(u, t) + u * diff(u, x) + v * diff(u, y)) - Integer(2) * e**2 * eta * diff(u, x, 2) - diff(u, y, 2)
            + e * zeta * diff(h, x) - diff(h, x, 3) - Integer(1)
            - e**2 * eta * diff(diff(u, x).subs(y, h), x)).subs(diff(h, t), - diff(q, x))
    pprint(integrate(xMomentumEquation * g0, (y, 0, h)).expand())
    print()

    pprint(integrate(diff(u, t) * g1, (y, 0, h)).expand())
    print()

    pprint((diff(diff(u, x).subs(y, h), x)*integrate(g1, (y, 0, h))).expand() + 2 * integrate(expand(diff(u, x, 2) * g1), (y, 0, h)).expand())
    print()

    pprint(integrate(diff(u, t) * g2, (y, 0, h)).expand())
    print()

    pprint((diff(diff(u, x).subs(y, h), x)*integrate(g2, (y, 0, h))).expand() + 2 * integrate(expand(diff(u, x, 2) * g2), (y, 0, h)).expand())
    print()


def wibl2TwoDimensionalOurEquations():
    x, y, t = symbols("x y t")
    h = Function("h")(x, t)
    q = Function("q")(x, t)
    r = Function("r")(x, t)
    ...
    b0 = Integer(3) / h * (q - e * r - e * s)
    b1 = Integer(45) / h * e * r
    b2 = Integer(210) / h * e * s

    u = b0 * g0 + b1 * g1 + b2 * g2

    vy = - diff(u, x)
    v = integrate(vy, y) # Using noslip as well

    xMomentumEquation = (e * Re * (diff(u, t) + u * diff(u, x) + v * diff(u, y)) - Integer(2) * e**2 * diff(u, x, 2) - diff(u, y, 2)
            + Integer(2) * e * Ct * diff(h, x) - C**-1 * diff(h, x, 3) - Integer(2)
...
def wibl2TwoDimensionalFallingLiquidFilmsSimplifiedShkadov():
    x, y, t = symbols("x y t")
    h = Function("h")(x, t)
    q = Function("q")(x, t)
    r = Function("r")(x, t)
    ...
    # Multiply solution by delta
    sol = (e * delta * solution[0]).expand()

    sol = (sol.coeff(e, 0) + e * sol.coeff(e, 1) + e**2 * sol.coeff(e, 2))
    
    f = open("wibl2SimplifiedShkadov.tex","w+")
    f.write(latex(sol))
    f.close()

    f = open("wibl2SimplifiedShkadov.txt","w+")
    f.write(str(sol))
    f.close()
...
def wibl2TwoDimensionalFallingLiquidFilmsSimplified():
    ...
    # These are different
    xMomentumEquation = (Integer(3) * e * Re * (diff(u, t) + u * diff(u, x) + v * diff(u, y)) - Integer(2) * e**2 * diff(u, x, 2) - diff(u, y, 2)
            + e * cottheta * diff(h, x) - C**-1 * diff(h, x, 3) - Integer(1)
            - e**2 * diff(diff(u, x).subs(y, h), x)).subs(diff(h, t), - diff(q, x))
    ...
    f = open("wibl2SimplifiedNusselt.tex","w+")
    f.write(latex(sol))
    f.close()

    f = open("wibl2SimplifiedNusselt.txt","w+")
    f.write(str(sol))
    f.close()
...
#        sol[n] = (sol[n].coeff(e, 0) + e * sol[n].coeff(e, 1) + e**2 * sol[n].coeff(e, 2)) + e**3 * sol[n].coeff(e, 3))
    ...
    for n in range(3):
        sol[n] = sol[n].subs(shkadovScaling).doit().subs(kappa**-2, eta) * delta
...
def wibl2TwoDimensionalFallingLiquidFilmsSkhadov():
    x, y, t = symbols("x y t")
    h = Function("h")(x, t)
    q = Function("q")(x, t)
    r = Function("r")(x, t)
    ...
    # Times solution by delta. So solving for (delta q_t).
    sol = [(delta * solution[0][n]).expand() for n in range(3)]
    
    f = open("wibl2FullSkhadov.tex","w+")
    for n in range(3):
        f.write(latex(sol[n]) + "\n")
    f.close()

    f = open("wibl2FullSkhadov.txt","w+")
    for n in range(3):
        f.write(str(sol[n])+ "\n")
    f.close()
